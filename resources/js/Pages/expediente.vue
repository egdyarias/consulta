<template>
  ...
  <AuthenticatedLayout>
    ...
    <div class="px-12 bg-white rounded-lg shadow-xs">
      ...
      <div class="m-6 mb-6 p-4 border shadow bg-sky-700 rounded-lg shadow-xs grid gap-6 md:grid-cols-2">
        ...
        <div class="space-y-3">
          ...
          <InputGroupLarge class="font-bold" id="antecedentes" :text="'Antecedentes del Paciente'"
            v-model="form2.antecedentes" :type="'text'" :maxlength="1000">
          </InputGroupLarge>
        ...
        <div class="space-y-3">
          ...
          <InputGroupLarge class="font-bold" id="diagnostico" :text="'Diagnóstico del Paciente'"
            v-model="form2.diagnostico" :type="'text'" :maxlength="1000">
          </InputGroupLarge>
      ...
      <div class="text-xl font-black text-center mb-4">Memoria Fotográfica<DarkButton class="ml-4"
          @click="openModalMemoria(1)">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-4">
            <path fill-rule="evenodd"
              d="M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s4.365 9.75 9.75 9.75 9.75-4.365 9.75-9.75S17.385 2.25 12 2.25ZM12.75 9a.75.75 0 0 0-1.5 0v2.25H9a.75.75 0 0 0 0 1.5h2.25V15a.75.75 0 0 0 1.5 0v-2.25H15a.75.75 0 0 0 0-1.5h-2.25V9Z"
              clip-rule="evenodd" />
      ...
      <div class="px-12 bg-white rounded-lg shadow-xs">
        <div class="overflow-hidden mb-8 w-full rounded-lg border shadow-xs">
          <div class="overflow-x-auto w-full">
            <table class="w-full whitespace-no-wrap">
              <thead>
                <tr class="text-xs font-semibold tracking-wide text-left text-white uppercase bg-sky-700 border-b">
                  <th class="px-4 py-3">Fecha</th>
                  <th class="px-4 py-3">Memoria</th>
                  <th class="px-4 py-3">Descripción</th>
                  <th class="px-4 py-3 flex justify-center">Opciones</th>
                </tr>
              </thead>
              <tbody class="bg-white divide-y">
                <tr v-for="memoria in memorias.data" :key="memoria.id" class="text-gray-700">
                  <td class="px-4 py-3 text-sm">
                    {{ formatearFecha(memoria.created_at) }}
                  </td>
                  <td class="px-4 py-3 text-sm font-semibold">
                    <a :href="'/storage/' + memoria.foto" target="_blank" rel="noopener noreferrer">
                      <img :src="'/storage/' + memoria.foto" class="max-h-[60px] cursor-pointer" />
                    </a>
                  </td>
                  <td class="px-4 py-3 text-sm font-semibold">
                    {{ memoria.descripcion }}
                  ...
                  <td class="px-4 py-3 text-sm flex justify-center gap-2">
                    <SecondaryButton @click="openModalVerArchivo(memoria)" class="mx-2">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="size-6">
                        <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <DangerButton @click="openModalDelMemoria(memoria)">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="size-6">
                        <path stroke-linecap="round" stroke-linejoin="round"
                          d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
          ...
          <div
            class="px-4 py-3 text-xs font-semibold tracking-wide text-gray-500 uppercase bg-gray-50 border-t sm:grid-cols-9">
            <pagination :links="memorias.links" />
          </div>
        </div>
      </div>
      ...
      <div class="m-6 mb-6 p-4 border shadow bg-sky-700 rounded-lg shadow-xs justify-center">
        ...
        <InputGroup class="font-bold" id="pendiente" :text="'Pendiente'" v-model="form2.pendiente" :type="'text'"
          :maxlength="1000">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
            stroke="currentColor" class="size-6">
            <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <Modal :show="showModalMemoria" @close="closeModalMemoria">
      <div class="" p-6>
        <h2 class="m-6 flex justify-center text-lg font-medium text-gray-900">{{ title }}</h2>
        <div class="m-6 flex justify-center">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
        ...
        <div class="ml-12 m-6 mb-6 space-y-6 max-w-xl">
          ...
          <FileInput :text="'Memoria'" :required="'required'" @change="handleFileChange" ref="fileInputRef" />
          <InputError class="mt-1" :message="formMemoria.errors.foto">
          </InputError>

          <InputGroup :text="'Nombre o Descripción de la Memoria'" :required="'required'"
            v-model="formMemoria.descripcion" :type="'text'" :maxlength="30">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
              stroke="currentColor" class="size-6">
              <path stroke-linecap="round" stroke-linejoin="round"
          ...
          <InputError class="mt-1" :message="formMemoria.errors.descripcion">
          </InputError>
          <div class="flex justify-center">
            <PrimaryButton @click="saveMemoria">
              Guardar
            </PrimaryButton>
          </div>
        </div>
      ...
      <div class="m-6 flex justify-center">
        <SecondaryButton @click="closeModalMemoria">Cancelar</SecondaryButton>
      </div>
    </Modal>

    <Modal :show="showModalDelMemoria" @close="closeModalDelMemoria">
      <div class="flex justify-center p-6">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
          class="size-24">
          <path stroke-linecap="round" stroke-linejoin="round"
      ...
      <div class="flex justify-center p-6 text-center">
        <p class="text-2xl text-gray-600"> ¿Estás seguro de eliminar esta Memoria Fotográfica?
        </p>
        <br>
      </div>
      <div class="m-6 flex justify-center">
        <PrimaryButton @click="deleteMemoria">
          Si, Eliminar.
        </PrimaryButton>
      </div>
    ...
    <Modal :show="showModalVerArchivo" @close="closeModalVerArchivo">
      <div class="">
        <h2 class="m-6 flex justify-center text-lg font-medium text-gray-900">{{ title }}</h2>
        <div class="px-4 py-3 text-sm">
          <a :href="'/storage/' + formVerArchivo.archivo" target="_blank" rel="noopener noreferrer">
            <img :src="'/storage/' + formVerArchivo.archivo" class="cursor-pointer" />
          </a>
        </div>
      </div>
      <div class="m-6 flex justify-center">
        <SecondaryButton @click="closeModalVerArchivo">Cancelar</SecondaryButton>
      </div>
    </Modal>
...
const showModalMemoria = ref(false);
const showModalDelMemoria = ref(false);


const openModalMemoria = (op) => {

  showModalMemoria.value = true;
  operation.value = op;
  if (op === 1) {
    formMemoria.paciente_id = paciente.id;
    title.value = 'Añadir Memoria Fotográfica';
  }
}
...
const closeModalMemoria = async () => {

  if (fileInputRef.value) {
    fileInputRef.value.$el.querySelector('input').value = '';
  }
  formMemoria.reset();
  formMemoria.clearErrors();
  showModalMemoria.value = false;
}

const openModalDelMemoria = (memoria) => {
  showModalDelMemoria.value = true;
  v.value.id = memoria.id
  v.value.nombre = memoria.descripcion
}

const closeModalDelMemoria = () => {
  showModalDelMemoria.value = false;
}
...
const handleFileChange = (event) => {
  const file = event.target.files[0]
  if (file) {
    formMemoria.foto = file
  }
}

async function saveMemoria() {

  formMemoria.post(route('memoria.store'), {
    preserveState: false,
    onSuccess: () => {
      formMemoria.reset();
      closeModalMemoria();
...
const deleteMemoria = () => {
  formMemoria.delete(route('memoria.destroy', v.value.id), {
    onSuccess: () => {
      formMemoria.reset();
      closeModalDelMemoria();
    }
  })

}

const formVerArchivo = useForm({
  archivo: ''
});

const showModalVerArchivo = ref(false);

const openModalVerArchivo = (memoria) => {

  showModalVerArchivo.value = true;
  title.value = memoria.descripcion;
  formVerArchivo.archivo = memoria.foto;
}

const closeModalVerArchivo = async () => {
  formVerArchivo.reset();
  showModalVerArchivo.value = false;
}


const abrirNuevaPestana = (formVerArchivo) => {
    const ruta = ref('/storage/' + formVerArchivo.archivo)
    const url = new URL(ruta.value, window.location.origin)
